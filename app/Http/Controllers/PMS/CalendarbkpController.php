                ...
                if ($propertyBooking) {
                    ...
                    if ($calendarDateTo < $checkoutDate) {
                        if ($calendarDateTo == $priceValue->price_date) {
                            ...
                            $propertyBookingConfirmed = PropertyBooking::where('checkin_date', $priceValue->price_date)->where('property_id', $id)->where('pType', 'unit')->where('property_booking_status', '!=', 'Canceled')->first();
                            if($propertyBookingConfirmed){
                                $className = 'start-date start-date-continuous';
                                $booking_class = 'bg-booked';
                                $is_booked = true;
                                $bookingId = $propertyBookingConfirmed->id;
                                $totalPrice = $propertyBookingConfirmed->payable_amount;
                                $no_of_guest = $propertyBookingConfirmed->no_of_adult;
                                $date_from = $checkinDate = date('Y-m-d', strtotime($propertyBookingConfirmed->checkin_date));
                                $date_to = $checkoutDate = date('Y-m-d', strtotime($propertyBookingConfirmed->checkout_date));
                                $customer_name = $propertyBookingConfirmed->customer_name;
                            }
                    ...
                    if($priceValue->price_date == $calendarDateFrom){
                        $propertyBookingConfirmed = DB::table('property_bookings')->where('checkin_date', '<=', $priceValue->price_date)->where('checkout_date', '>=', $priceValue->price_date)->where('pType', 'unit')->where('property_id', $id)->where('property_booking_status', '!=', 'Canceled')->first();
