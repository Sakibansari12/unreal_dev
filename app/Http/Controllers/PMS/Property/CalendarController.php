...
class CalendarController extends Controller{
    ...
    //     $remainingDays = $lastDayOfMonth - date('j') + 1;
    ...
    //                 }
    //             }
    //             $minSatyDetail = RuPropertyMinstay::where('minstay_date', $priceValue->price_date)->where('type', 'unit')->where('ru_property_id', $detail->ru_property_id)->first();
    //             $minSaty = 1;
    //             if($minSatyDetail){
    ...
    public function calendarModalFormSubmit(Request $request){
        ...
        elseif($request['type'] =='priceChange'){
            ...
            if($request['formdata']['minNights'] !=''){
                RuPropertyMinstay::whereBetween('minstay_date', [$request['formdata']['date_from'], $request['formdata']['date_to']])
                ->where('ru_property_id', $request['formdata']['propertyId'])
                ->update(['is_minstay_count' => $request['formdata']['minNights']]);
            }
    ...
    public function calendarBlockDates($formData){
        ...
        $xml = "<Push_PutAvbUnits_RQ>
            ...
            <MuCalendar PropertyID='".$property->ru_property_id."'>
                <Date From='".date('Y-m-d', strtotime($date_from))."' To='".date('Y-m-d', strtotime($date_to))."'>
                    <U>0</U>
                    <C>4</C>
                </Date>
            </MuCalendar>
