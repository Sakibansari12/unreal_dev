...
class UserController extends Controller
{
    ...
    public function form($id = null)
    {
        ...
        if ($detail && $detail->role === 'Owners') {
            $userMappedPropertyIds = TblHomeUnit::where('user_id', $detail->id)->pluck('id')->toArray();
            $properties = TblHomeUnit::where(function ($q) use ($userMappedPropertyIds) {
                $q->whereNull('user_id')->orWhereIn('id', $userMappedPropertyIds);
            })->get();
        } else {
            $properties = TblHomeUnit::whereNull('user_id')->get();
        }
    ...
    public function save(Request $request)
    {
        ...
        if ($role->role_name === 'Owners') {
            TblHomeUnit::where('user_id', $data->id)->update(['user_id' => null]);
            if ($request->has('mappedProperties') && is_array($request->mappedProperties)) {
                TblHomeUnit::whereIn('id', $request->mappedProperties)->update(['user_id' => $data->id]);
            }
        } else {
            TblHomeUnit::where('user_id', $data->id)->update(['user_id' => null]);
        }
    ...
    public function delete($id)
    {
        try {
            ...
            if ($user->role === 'Owners') {
                TblHomeUnit::where('user_id', $user->id)->update(['user_id' => null]);
            }
            $user->delete();
    ...
    public function multiDelete(Request $request)
    {
        ...
        try {
            ...
            if (!empty($owners)) {
                TblHomeUnit::whereIn('user_id', $owners)->update(['user_id' => null]);
            }
