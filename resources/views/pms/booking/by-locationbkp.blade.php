                                            ...
                                            <div class="col-12">
                                                <div class="form-fields">
                                                    <div class="row gx-2 gx-md-3">
                                                        ...
                                                        <div class="col-12 col-sm-6 col-lg-3">
                                                            ...
                                                            <div class="input-group flex-nowrap">
                                                                <select id="countryCode" class="form-control form-select pe-4" name="country_code" style="width:120px; font-size:13px !important">
                                                    ...
                                                    <div class="row">
                                                        <div class="col-6 col-lg-3">
                                                            <div class="form-field">
                                                                <label for="checkInTime">Arrival time</label>
                                                                <input type="time" name="check-in-time" id="checkInTime" class="form-control flatpickr">
                                                            </div>
                                                        </div>
                                                        <div class="col-6 col-lg-3">
                                                            <div class="form-field">
                                                                <label for="checkOutTime">Departure time</label>
...
$(document).ready(function() {
    ...
    try {
        ...
        function onHomeRadio(e, id, idx) {
            ...
            let homeListFilter = homeList.filter((item) => {
                if (id == item.id) {
                    ...
                    pType = item.pType || '';
                    // Check if no_adults >= guests_included and no_adults <= maximum_number_of_guests
                    if (totalGuest >= guestIncluded && totalGuest <= (item.maximum_number_of_guests || 20)) {
                        extraAddedGuestNumber = totalGuest - guestIncluded;
                        extraGuestCharge = extraGuestChargePerNight * extraAddedGuestNumber * noOfNights;
                        price = item.price;
                        totalAmount = price + extraGuestCharge;
                        taxAmount = (totalAmount * tax) / 100;
                    } else {
                        extraAddedGuestNumber = 0;
                        extraGuestCharge = 0;
                        price = item.price;
                        totalAmount = price;
                        taxAmount = (totalAmount * tax) / 100;
            ...
            // Populate no_adults dropdown based on maximum_number_of_guests
            let maxGuests = homePrices.maximum_number_of_guests || 20; // Fallback to 20 if not defined
            let noAdultsSelect = $('#no_adults');
            noAdultsSelect.empty(); // Clear existing options
            for (let i = 1; i <= maxGuests; i++) {
                noAdultsSelect.append(`<option value="${i}" ${i == no_adults ? 'selected' : ''}>${i}</option>`);
            }
        ...
        function renderPriceDetails() {
            ...
            const originalTotal = Number(price || 0) + Number(extraGuestCharge || 0);

            // Ensure totalAmount is calculated from originalTotal
            totalAmount = originalTotal - Number(discountAmount || 0);

            // Ensure totalAmount is not negative
            if (totalAmount < 0) {
                totalAmount = 0;
                discountAmount = originalTotal; // Cap discount at originalTotal
            }
            ...
            let html = `
                ...
                <tr>
                    <td>Base Price:</td>
                    <td>₹${currFormat(price || 0)}</td>
                </tr>
            `;

            // Add Extra Guest Charges row if extraGuestCharge > 0
                ...
                <tr>
                    ...
                    <td>
                        <div class="input-group small-input-group">
                            ...
                            <input type="number" name="discount_amount" class="form-control" value="${discountAmount || ''}" data-max="${originalTotal.toFixed()}" min="0">
                            <div class="invalid-feedback">The value should be less than or equal to ₹${currFormatTotal(originalTotal)}</div>
                        </div>
                    </td>
                </tr>
                <tr>
                    <th>Sub Total:</th>
                    <th>₹${currFormatTotal(totalAmount)}</th>
                </tr>
            `;
            ...
            $('input[name="discount_amount"]').off('input change keydown focus blur').on({
                ...
                blur: function() {
                    ...
                    // If value is invalid or empty, clear the field and reset discount
                    if (Number(input.value) > max || !input.value) {
                        input.value = ''; // Clear the input
                        discountAmount = 0; // Reset discountAmount
                        totalAmount = originalTotal; // Reset totalAmount
                        input.classList.remove('is-invalid'); // Remove error styling
                        $(input).next('.invalid-feedback').hide(); // Hide error message
                    }

                    renderPriceDetails(); // Render to update table
                }
            });

            // Rebind event listeners for add_ons_discount_amount
            $('input[name="add_ons_discount_amount"]').off('input change keydown focus blur').on({
                input: function(e) {
                    ...
                    // Update addOnsDiscountAmount only if valid
                    if (newValue > max) {
                        input.classList.add('is-invalid');
                        $(input).next('.invalid-feedback').show();
                        adOnsDiscountAmount = 0; // Reset adOnsDiscountAmount
                        addOnsDiscountTotalAmount = addOnsTotalAmount; // Reset addOnsDiscountTotalAmount
                    } else {
                        input.classList.remove('is-invalid');
                        $(input).next('.invalid-feedback').hide();
        ...
        // Handle no_adults change
        $('#no_adults').on('change', function() {
            no_adults = $(this).val();
            if (vHomeId) {
                // Re-run onHomeRadio to recalculate prices
                onHomeRadio(null, vHomeId, $('input[name="home[]"]:checked').index());
            }
        });
        ...
        $('#bookingFormBtn').on('click', function(e) {
            ...
            let formData = {
                ...
                tax: tax,
                taxAmount: taxAmount,
                website_markup_price: homePrices.website_markup_price,
