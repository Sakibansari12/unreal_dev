    ...
    <div class="container-fluid">
        ...
        <div class="content-box p-3">
            <div class="row">
                ...
                <div class="col-12 pt-3">
                    <div class="bg-dark text-white py-2 px-3 text-center">
                        ...
                        ₹<span id="remainingAmount">{{ number_format_indian($remainingAmount + $paymentRequestData->amount) }}</span>
                    </div>
                </div>
            </div>
            <div class="border pb-2 mt-4">
                ...
                <!-- <button type="button" class="btn btn-warning btn-sm mt-2 mx-2 text-white py-2 px-4" id="add-row-btn" disabled>+ Add</button> -->
            </div>
        </div>
    </div>
</section>
<script>
    let totalPayableAmount = '{{ $remainingAmount + $paymentRequestData->amount }}';
    let rowIndex = 1;
    ...
    function calculateRemainingAmount() {
        let total = parseFloat("{{ $remainingAmount + $paymentRequestData->amount }}");
        let used = 0;
                ...
                if (res.status) {
                    ...
                    if (row.find('.edit-row').length === 0) {
                        row.find('td:last').append(`
                            <button type="button" class="btn btn-primary btn-small edit-row ms-1">Edit
                            </button>
                        `);
                    }
    ...
    $(document).on('click', '.update-row', function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.status) {
                    row.find('input, select').prop('disabled', true);
                    updateBtn.replaceWith(`<button type="button" class="btn btn-primary btn-small edit-row">Edit
                                </span></button>`);
                    calculateRemainingAmount();
                    toastr.success(res.message, 'Success', { timeOut: 2000 });
                } else {
