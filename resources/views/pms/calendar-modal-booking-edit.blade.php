...
<div class="modal-body form-box">
    <h4 class="text-primary fs-6 mb-0">{{ $bookingDetail->property->unit_name }}</h4>
    <br>
    <br>
    <form action="#" id="calendarModalFormBooking"  novalidate>
        <input type="hidden" name="date_from" value="{{ date('Y-m-d', strtotime($bookingDetail->checkin_date)) }}">
        <input type="hidden" name="date_to" value="{{ date('Y-m-d', strtotime($bookingDetail->checkout_date)) }}">
        <input type="hidden" name="propertyId" value="{{ $bookingDetail->property->property_unique_id }}">
        ...
        <div class="modal-footer">
            <button type="button" class="btn btn-primary booking-submit"  onclick="submitBookingForm()"><span>Save</span></button>
        </div>
    </form>
</div>
