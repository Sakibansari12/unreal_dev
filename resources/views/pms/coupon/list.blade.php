    ...
    <div class="container-fluid">
        <div class="title">
            <div class="row gx-2 align-items-center">
                <div class="col">
                    <h1 class="fs-5 mb-0">Coupon List</h1>
                </div>
                <div class="col-auto">
                    <a href="{{ route('pms.coupon.form') }}" class="btn d-flex btn-small rounded-2 btn-secondary">
        ...
        <div class="content-box p-3">
            <form method="GET" action="{{ route('pms.coupon.list') }}">
                @csrf
                <div class="row mb-3 g-3 flex-sm-row-reverse align-items-center">
                    <div class="col-sm-6 col-md-5 col-lg-4">
                        <div class="input-group input-group-sm">
                            ...
                            <a href="{{ route('pms.coupon.list') }}" class="btn btn-warning">
                    ...
                    <div class="col">
                        <div class="data-info text-primary">
                            {{ $items->total() }} {{ $items->total() == 1 ? 'Result' : 'Results' }} found
                        </div>
                    </div>
                </div>
            </form>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex align-items-center gap-2">
                                    <!--<span  class="text-primary fw-semibold copy-btn" data-code="{{ explode(',', $value->codes)[0] }}">-->
                                    <!--    {{ explode(',', $value->codes)[0] }}-->
                                    <!--</span>-->
                                    
                                    <span
                                        class="text-primary fw-semibold copy-btn text-decoration-none" style="cursor: pointer;"
                                        onclick="copyToClipboard(this)"
                                        data-bs-toggle="tooltip"
                                        data-bs-placement="top"
                                        data-text="{{ explode(',', $value->codes)[0] }}"
                                        title="Copy">
                                        {{ explode(',', $value->codes)[0] }}
                                    </span>
                                    ...
                                    @if(count(explode(',', $value->codes)) > 1)
                                        
                                <a href="{{ route('pms.couponcode.export', [
                                    'id' => $value->id,
								]) }}"
                                    class="btn-export">
                            ...
                            <td>{{ $value->discount }}{{ $value->discount_type == 'flat' ? '' : '%' }}</td>
                            <td>
                            ...
                            <td>
                                {{ \Carbon\Carbon::parse($value->stay_date_from)->format('d M, y') }}
                                to
                                {{ \Carbon\Carbon::parse($value->stay_date_to)->format('d M, y') }}
                            </td>
                            <td>
                            ...
                            <td>
                                <ul class="actions">
                                    <li><a href="{{ route('pms.coupon.form', ['id'=>$value->id]) }}" class="btn btn-link"><i class="icon-edit"></i></a></li>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        $(".deleteSelected").click(function () {
            ...
            if (idArray.length > 0) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ url("pms/coupon/multidelete") }}',
                            method: 'POST',
                            data: { ids: idArray },
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
        ...
        $('.btn_status').on('click', function () {
            ...
            $.ajax({
                url: '{{ url("pms/coupon/toggle-status") }}/' + id,
                type: 'PUT',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
                },
