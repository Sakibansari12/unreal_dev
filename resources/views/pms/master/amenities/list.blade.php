    ...
    <div class="container-fluid">
        <div class="title">
            <div class="row gx-2 align-items-center">
                ...
                <div class="col-auto">
                    <a href="{{ route('pms.amenities.form') }}" class="btn d-flex btn-small rounded-2 btn-secondary">
        ...
        <div class="content-box p-3">
            <form method="GET" action="{{ route('pms.amenities.list') }}">
                @csrf
                <div class="row mb-3 g-3 flex-sm-row-reverse align-items-center">
                    <div class="col-sm-6 col-md-5 col-lg-4">
                        <div class="input-group input-group-sm">
                            ...
                            <a href="{{ route('pms.amenities.list') }}" class="btn btn-warning">
                ...
                <table class="table align-middle">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @php
                                $imagePath = public_path('storage/amenities/' . ($value->amenities_image ?? ''));
                                @endphp
                                @if(!empty($value->amenities_image) && file_exists($imagePath))
                                <img src="{{ asset('storage/amenities/' . $value->amenities_image) }}" style="width: 50px;" alt="Amenity Icon">
                                @else
                            ...
                            <td>
                                <ul class="actions">
                                    <li><a href="{{ route('pms.amenities.form', ['id'=>$value->id]) }}" class="btn btn-link"><i class="icon-edit"></i></a></li>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        $(document).on('click', '.item_delete', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ url("pms/amenities/delete") }}/' + id,
                        method: "DELETE",
                        headers: {
        ...
        $(".deleteSelected").click(function () {
            ...
            if (idArray.length > 0) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ url("pms/amenities/multidelete") }}',
                            method: 'POST',
                            data: { ids: idArray },
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
        ...
        $('.btn_status').on('click', function () {
            ...
            $.ajax({
                url: '{{ url("pms/amenities/toggle-status") }}/' + id,
                type: 'PUT',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
                },
