...
<style>
    ...
    .upload-mobile-wrapper input[type=file] {
        position: absolute;
        height: 100%;
        width: 100%;
        opacity: 0;
    ...
    .upload-mobile-wrapper.is-invalid {
        border-color: #dc3545;
    }
        ...
        <div class="content-box p-3">
            <div class="form-box">
                <form action="{{ route('pms.bannerslide.save', $detail->id ?? '') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <input type="hidden" name="mobile_remove_image" id="removeMobileImageInput" value="0">
                        ...
                        <div class="row">
                            <div class="col-6">
                                <label for="mobile_image">Mobile Image{{-- <sup>*</sup> --}}</label>
                                <div class="upload-mobile-wrapper @error('mobile_image') is-invalid @enderror" id="uploadWrapper">
                                    <input type="file" name="mobile_image" id="mobileImageInput" accept="image/*" onchange="previewMobileImage(event)">
                                    <div class="upload-mobile-info" id="mobileUploadInfo" onclick="document.getElementById('mobileImageInput').click()" style="@if(isset($detail->mobile_image)) display:none; @endif">
                                        <i class="bi bi-upload fs-4"></i>
                                        <div class="upload-info-text">
                                            <strong><span class="text-primary">Browse</span> Your File.</strong>
                                            <!-- <small class="d-block">Max size: 3MB | Image size: 100x100px</small> -->
                                    ...
                                    <div id="mobileImagePreview" class="imagePreview @if(!isset($detail->mobile_image)) d-none @endif">
                                        @php
                                            $imagePath = public_path('storage/home_banner/' . ($detail->mobile_image ?? ''));
                                        @endphp
                                        @if(!empty($detail->mobile_image) && file_exists($imagePath))
                                            <img src="{{ asset('storage/home_banner/' . $detail->mobile_image) }}" alt="Home Banner Image">
                                        @endif
                                    </div>
                                    <button type="button" class="remove-mobile-preview @if(!isset($detail->mobile_image)) d-none @endif" onclick="removeMobileImage()">×</button>
                                </div>
                                @error('mobile_image')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
...
<script>
    ...
    function previewMobileImage(event) {
        ...
        const info = document.getElementById('mobileUploadInfo');
        const removeBtn = document.querySelector('.remove-mobile-preview');
