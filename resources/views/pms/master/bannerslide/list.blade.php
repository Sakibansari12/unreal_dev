    ...
    <div class="container-fluid">
        <div class="title">
            <div class="row gx-2 align-items-center">
                <div class="col">
                    <h1 class="fs-5 mb-0">Hero Slide List</h1>
                </div>
                @if($items->total() < 1)
                <div class="col-auto">
                    <a href="{{ route('pms.bannerslide.form') }}" class="btn d-flex btn-small rounded-2 btn-secondary">
        ...
        <div class="content-box p-3">
            <form method="GET" action="{{ route('pms.bannerslide.list') }}">
                @csrf
                <div class="row mb-3 g-3 flex-sm-row-reverse align-items-center">
                    <div class="col-sm-6 col-md-5 col-lg-4">
                        <div class="input-group input-group-sm">
                            <input type="text" name="search_heading" class="form-control" placeholder="Search by title." value="{{ request('search_heading') }}">
                            <button type="submit" class="btn btn-primary">
                                <i class="icon-search"></i>
                            </button>
                            <a href="{{ route('pms.bannerslide.list') }}" class="btn btn-warning">
                ...
                <table class="table align-middle">
                    ...
                    <tbody id="sortable">
                        ...
                        <tr data-id="{{ $value->id }}">
                            ...
                            <td>
                                @php
                                $imagePath = public_path('storage/home_banner/' . ($value->image ?? ''));
                                @endphp
                                @if(!empty($value->image) && file_exists($imagePath))
                                <img src="{{ asset('storage/home_banner/' . $value->image) }}" alt="Home Banner Image" width="100px">
                                @else
                            ...
                            <td>
                                @php
                                $imagePath = public_path('storage/home_banner/' . ($value->mobile_image ?? ''));
                                @endphp
                                @if(!empty($value->mobile_image) && file_exists($imagePath))
                                <img src="{{ asset('storage/home_banner/' . $value->mobile_image) }}" alt="Home Banner Mobile Image" width="100px">
                                @else
                                <svg width="70" height="70" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <rect width="24" height="24" rx="4" fill="#f0f0f0"/>
                                    <path d="M4 17L8 13L11 16L16 11L20 15V19H4V17Z" fill="#ccc"/>
                            ...
                            <td>
                                <ul class="actions">
                                    <li><a href="{{ route('pms.bannerslide.form', ['id'=>$value->id]) }}" class="btn btn-link"><i class="icon-edit"></i></a></li>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        $(document).on('click', '.item_delete', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ url("pms/bannerslide/delete") }}/' + id,
                        method: "DELETE",
                        headers: {
        ...
        $(".deleteSelected").click(function () {
            ...
            if (idArray.length > 0) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ url("pms/bannerslide/multidelete") }}',
                            method: 'POST',
                            data: { ids: idArray },
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
        ...
        $('.btn_status').on('click', function () {
            ...
            $.ajax({
                url: '{{ url("pms/bannerslide/toggle-status") }}/' + id,
                type: 'PUT',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
                },
        ...
        $("#sortable").sortable({
            ...
            update: function (event, ui) {
                ...
                $.ajax({
                    url: '{{ route("pms.bannerslide.position") }}',
                    type: 'POST',
