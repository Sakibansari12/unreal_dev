    ...
    <div class="container-fluid">
        <div class="title">
            <div class="row gx-2 align-items-center">
                <div class="col">
                    <h1 class="fs-5 mb-0">Blog List</h1>
                </div>
                <div class="col-auto">
                    <a href="{{ route('pms.blog.form') }}" class="btn d-flex btn-small rounded-2 btn-secondary">
        ...
        <div class="content-box p-3">
            <form method="GET" action="{{ route('pms.blog.list') }}">
                <div class="row mb-3 g-3 flex-sm-row-reverse align-items-center">
                    <div class="col-sm-6 col-md-5 col-lg-4">
                        <div class="input-group input-group-sm">
                            <input type="text" name="search" class="form-control" placeholder="Search by Blog Title" value="{{ request('search') }}">
                            <button type="submit" class="btn btn-primary">
                                <i class="icon-search"></i>
                            </button>
                            <a href="{{ route('pms.blog.list') }}" class="btn btn-warning">
                ...
                <table class="table align-middle">
                    ...
                    <tbody id="sortable">
                        ...
                        <tr data-id="{{ $value->id }}">
                            ...
                            <td>
                                @php
                                $imagePath = public_path('storage/blog/' . ($value->image ?? ''));
                                @endphp
                                @if(!empty($value->image) && file_exists($imagePath))
                                <img src="{{ asset('storage/blog/' . $value->image) }}" alt="Blog Image" width="100px">
                                @else
                            ...
                            <td>
                                <ul class="actions">
                                    <li><a href="{{ route('pms.blog.form', ['id'=>$value->id]) }}" class="btn btn-link"><i class="icon-edit"></i></a></li>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        $(document).on('click', '.item_delete', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ url("pms/blog/delete") }}/' + id,
                        method: "DELETE",
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
                        },
                        success: function (response) {
                            Swal.fire('Deleted!', response.message || 'The blog has been deleted.', 'success');
        ...
        $('.btn_status').on('click', function () {
            ...
            $.ajax({
                url: '{{ url("pms/blog/toggle-status") }}/' + id,
                type: 'PUT',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
                },
