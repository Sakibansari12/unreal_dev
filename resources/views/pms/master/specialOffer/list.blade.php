    ...
    <div class="container-fluid">
        <div class="title">
            <div class="row gx-2 align-items-center">
                <div class="col">
                    <h1 class="fs-5 mb-0">Special Offer List</h1>
                </div>
                <div class="col-auto">
                    <a href="{{ route('pms.specialoffer.form') }}" class="btn d-flex btn-small rounded-2 btn-secondary">
        ...
        <div class="content-box p-3">
              <form method="GET" action="{{ route('pms.specialoffer.list') }}">
                <div class="row mb-3 g-3 flex-sm-row-reverse align-items-center">
                    <div class="col-sm-6 col-md-5 col-lg-4">
                        <div class="input-group input-group-sm">
                            <input type="text" name="search" class="form-control" placeholder="Search by Offer Name" value="{{ request('search') }}">
                            <button type="submit" class="btn btn-primary">
                                <i class="icon-search"></i>
                            </button>
                            <a href="{{ route('pms.specialoffer.list') }}" class="btn btn-warning">
                    ...
                    <div class="col">
                        <div class="data-info text-primary">
                            {{ $specialInvitations->total() }} Result{{ $specialInvitations->total() == 1 ? '' : 's' }} found
                        </div>
                    </div>
                </div>
            </form>
            ...
            <div class="table-responsive data-table text-nowrap">
                <table class="table align-middle">
                    ...
                    <tbody id="">
                        @forelse($specialInvitations as $key => $value)
                        <tr data-id="{{ $value->id }}">
                            <td>
                            ...
                            <td>
                                @php
                                $imagePath = public_path('storage/specialoffer/' . ($value->image ?? ''));
                                @endphp
                                @if(!empty($value->image) && file_exists($imagePath))
                                <img src="{{ asset('storage/specialoffer/' . $value->image) }}" alt="Offer Image" style="width: 50px;" width="100px">
                                @else
                            ...
                            <td>{{ $value->couponCode->coupon_code ?? 'Coupon #' . $value->couponcode_id }}</td>
                            <td>{{ $value->headline }}</td>
                            <td>{{ \Carbon\Carbon::parse($value->validity)->format('d/m/Y') }}</td>
                            ...
                            <td>
                                <ul class="actions">
                                    <li><a href="{{ route('pms.specialoffer.form', ['id' => $value->id]) }}" class="btn btn-link"><i class="icon-edit"></i></a></li>
            ...
            <div class="mt-3">
                {{ $specialInvitations->links('vendor.pagination.bootstrap-5') }}
            </div>
        </div>
    </div>
</section>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    $(document).on('click', '.item_delete', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ url("pms/specialoffer/delete") }}/' + id,
                    method: "DELETE",
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
                    },
                    success: function () {
                        Swal.fire('Deleted!', 'The special offer has been deleted.', 'success');
    ...
    $(".deleteSelected").click(function () {
        ...
        if (idArray.length > 0) {
            Swal.fire({
                title: 'Are you sure?',
                text: 'Selected special offers will be deleted!',
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ url("pms/specialoffer/multidelete") }}',
                        method: 'POST',
                        data: { ids: idArray },
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
                        ...
                        success: function () {
                            Swal.fire('Deleted!', 'Selected special offers have been deleted.', 'success');
                            setTimeout(() => location.reload(), 1500);
                        },
                        error: function () {
                            Swal.fire('Error', 'Something went wrong!', 'error');
    ...
    $('.btn_status').on('click', function () {
        ...
        $.ajax({
            url: '{{ url("pms/specialoffer/toggle-status") }}/' + id,
            type: 'PUT',
            data: { status: status },
            headers: {
                'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
    ...
    $("#sortable").sortable({
        ...
        update: function (event, ui) {
            ...
            $.ajax({
                url: '{{ route("pms.specialoffer.position") }}',
                type: 'POST',
                ...
                success: function () {
                    toastr.success('Positions updated successfully.', 'Success', { timeOut: 2000 });
                },
                error: function () {
                    toastr.error('Something went wrong!', 'Error', { timeOut: 2000 });
                }
