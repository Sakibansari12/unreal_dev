        ...
        <div class="container-fluid">
            <div class="title">
                <div class="row gx-2 align-items-center">
                    <div class="col">
                        <h1 class="fs-5 mb-0">Tags List</h1>
                    </div>
                    <div class="col-auto">
                        <a href="{{ route('pms.tags.form') }}" class="btn d-flex btn-small rounded-2 btn-secondary">
            ...
            <div class="content-box p-3">
                <form method="GET" action="{{ route('pms.tags.list') }}">
                    <div class="row mb-3 g-3 flex-sm-row-reverse align-items-center">
                        <div class="col-sm-6 col-md-5 col-lg-4">
                            <div class="input-group input-group-sm">                    
                                ...
                                <a href="{{ route('pms.tags.list') }}" class="btn btn-warning">
                    ...
                    <table class="table align-middle">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                @php
                                $imagePath = public_path('storage/tag/' . ($value->tags_image ?? ''));
                                @endphp
                                @if(!empty($value->tags_image) && file_exists($imagePath))
                                <img src="{{ asset('storage/tag/' . $value->tags_image) }}" style="width: 50px;" alt="tag" width="100px">
                                @else
                                ...
                                <div class="form-check form-switch">
                                    <input class="form-check-input btn_show_home" type="checkbox" role="switch" data-value="{{ $value->id }}" {{ $value->tag_show_on_page ? 'checked' : '' }}>
                                </div>
                            </td>
                                <td>
                                    <div class="form-check form-switch">
                                ...
                                <td>
                                    <ul class="actions">
                                        <li><a href="{{ route('pms.tags.form', ['id'=>$value->id]) }}" class="btn btn-link"><i class="icon-edit"></i></a></li>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            $('.btn_status').on('click', function () {
                ...
                $.ajax({
                    url: '{{ url("pms/tags/toggle-status") }}/' + id,
                    type: 'post',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
                    },
            ...
            $('.btn_show_home').on('click', function () {
                ...
                $.ajax({
                    url: '{{ url("pms/tags/show_home/toggle-status") }}/' + id,
                    type: 'PUT',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
                    },
