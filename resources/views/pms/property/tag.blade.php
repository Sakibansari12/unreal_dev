        ...
        <div class="content-box p-3">
            <div class="row g-5">
                ...
                <div class="col-12 col-lg-9">
                    <form method="POST" action="{{ route('pms.property.unit.or.multiunit.tag.save') }}">
                        @csrf
                                ...
                                <div class="form-group">
                                    <div class="form-check form-check-lg form-check-box {{ $item['isChecked'] ? 'border-primary' : '' }}">
                                        <div class="row gx-3 gy-3 align-items-center">
                                            ...
                                            <div class="col">
                                                <img
                                                    src="{{ asset('storage/tag/' . $item['tags_image']) }}"
                                                    height="20">&nbsp;
                                                <label for="amenity_{{ $item['id'] }}">{{ $item['tags_name'] }}</label>
                                            </div>

                                            <!-- HIDDEN FIELDS -->
                                            <input type="hidden" name="amenities[{{ $amenityIndex }}][tags_id]" value="{{ $item['id'] }}">
                                            <input type="hidden" name="amenities[{{ $amenityIndex }}][tags_name]" value="{{ $item['tags_name'] }}">
                                            <input type="hidden" class="position-input" name="amenities[{{ $amenityIndex }}][position]" value="{{ $index }}">

                                            <div class="col-12 d-none">
                                                <input
                                                    type="number"
                                                    name="amenities[{{ $amenityIndex }}][tags_number]"
                                                    class="form-control"
                                                    value="{{ $item['number'] ?? '' }}"
