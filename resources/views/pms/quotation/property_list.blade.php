                    ...
                    <table class="table table-borderless">
                        <tbody>
                            <tr>
                                ...
                                <td align="end" class="price-per-night">Rs. {{ formatIN($property['price_per_night'] ?? 0, 0) }}</td>
                            </tr>
                            <tr>
                                <td>Number of Nights:</td>
                                <td align="end">{{ $property['no_of_nights'] ?? 0 }}</td>
                            </tr>
                            <tr>
                                <td>Base Price:</td>
                                <td align="end" class="base-price">Rs. {{ formatIN($property['base_price'] ?? 0, 0) }}</td>
                            </tr>
                            @if($property['total_extra_guest_charge'] != 0)
                            <tr>
                                <td>Extra Guest:</td>
                                <td align="end" class="extraGuestCharge">Rs. {{ formatIN($property['total_extra_guest_charge'] ?? 0, 0) }}</td>
                            </tr>
                            @endif
                            <tr>
                                <td>Discount:</td>
                            ...
                            <tr>
                                <td>Sub Total:</td>
                                <td align="end" class="sub-total">Rs. {{ formatIN($property['sub_total'] ?? 0, 0) }}</td>
                            </tr>
                            ...
                            @foreach($property['additionalCharge'] as $obj)
                                <tr>
                                    <td>{{ $obj['name'] ?? '' }}:</td>
                                    <td align="end" class="addon-price" data-addon-price="{{ $obj['price'] ?? 0 }}">Rs. {{ formatIN($obj['price'] ?? 0, 0) }}</td>
                                </tr>
                            @endforeach
                            <tr>
                                <td>Discount:</td>
                                <td align="end">
                                    <div class="input-group small-input-group">
                                        ...
                                        <input
                                            ...
                                            @if(!isset($property['selected']) || !$property['selected']) disabled @endif
                                            min="0"
                                            max="{{ $property['total_additional_charges'] ?? 0 }}"
                                        />
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                <td>Sub Total:</td>
                                <td align="end" class="addons-sub-total">Rs. {{ formatIN(($property['total_additional_charges'] ?? 0) - (isset($property['discount_amount_additional']) ? $property['discount_amount_additional'] : 0), 0) }}</td>
                            </tr>
                            @endif
                            <tr>
                                <th>Total Taxable Amount:</th>
                                <td align="end" class="total-taxable-amount">Rs. {{ formatIN(($property['total_taxable_amount'] ?? 0) - (isset($property['discount_amount']) ? $property['discount_amount'] : 0) - (isset($property['discount_amount_additional']) ? $property['discount_amount_additional'] : 0), 0) }}</td>
                            </tr>
                            <tr>
                                <!--<td>GST ( {{ $property['tax_percentage'] ?? 0 }}%):</td>-->
                                <td id="gst-rate">GST ( {{ $property['tax_percentage'] ?? 0 }}%):</td>
                                <td align="end" class="tax-amount">Rs. {{ formatIN($property['tax_amount'] ?? 0, 0) }}</td>
                            </tr>
                            <tr>
                                <td>Total Amount Payable:</td>
                                <td align="end" class="final-price">Rs. {{ formatIN(($property['price'] ?? 0) - (isset($property['discount_amount']) ? $property['discount_amount'] : 0) - (isset($property['discount_amount_additional']) ? $property['discount_amount_additional'] : 0), 0) }}</td>
                            </tr>
                            <input type="hidden" class="original-subtotal" value="{{ $property['sub_total'] ?? 0 }}">
                            <input type="hidden" class="original-website-markup-price" value="{{ $property['website_markup_price'] ?? 0 }}">
...
$(document).ready(function () {
    ...
    function updateCardCalculations($card, isManualUpdate = false) {
        ...
        let taxPercentage = parseFloat($card.find('.tax-percentage').val()) || 0;
        ...
        if (discount1 > baseCalSub) {
            $card.find('.discount-1').after('<div class="text-danger discount-error mt-1">Discount cannot exceed Rs. ' + Math.round(baseCalSub).toLocaleString('en-IN', { maximumFractionDigits: 0 }) + '</div>');
            enableSubmitButton = false;  // Disable submit button
        }
        if (discount2 > totalAddonsPrice) {
            $card.find('.discount-2').after('<div class="text-danger discount-error mt-1">Discount cannot exceed Rs. ' + Math.round(totalAddonsPrice).toLocaleString('en-IN', { maximumFractionDigits: 0 }) + '</div>');
        ...
        updatedBaseSubtotal = Math.round(updatedBaseSubtotal);
        ...
        updatedAdditionalCharges = Math.round(updatedAdditionalCharges);

        const totalTaxableAmount = Math.round(updatedBaseSubtotal + updatedAdditionalCharges);
        
        taxPercentage = 5;
        if(totalTaxableAmount > 7500){
            taxPercentage = 18;
            document.getElementById("gst-rate").innerText = `GST (${taxPercentage}%):`;
            document.querySelector(".tax-percentage").value = taxPercentage;
        ...
        const taxAmount = Math.round((totalTaxableAmount * taxPercentage) / 100);
        const finalPrice = Math.round(totalTaxableAmount + taxAmount);

        // DOM update
        $card.find('.sub-total').text('Rs. ' + updatedBaseSubtotal.toLocaleString('en-IN', { maximumFractionDigits: 0 }));
        $card.find('.addons-sub-total').text('Rs. ' + updatedAdditionalCharges.toLocaleString('en-IN', { maximumFractionDigits: 0 }));
        $card.find('.total-taxable-amount').text('Rs. ' + totalTaxableAmount.toLocaleString('en-IN', { maximumFractionDigits: 0 }));
        $card.find('.tax-amount').text('Rs. ' + taxAmount.toLocaleString('en-IN', { maximumFractionDigits: 0 }));
        $card.find('.final-price').text('Rs. ' + finalPrice.toLocaleString('en-IN', { maximumFractionDigits: 0 }));
    }
