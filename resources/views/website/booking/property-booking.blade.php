                        ...
                        <div class="row gx-3 align-items-baseline">
                            <div class="col-12 col-lg-auto">
                                <div class="back-nav">
                                    ...
                                    'tot_no_of_days'=>request()->input('tot_no_of_days'),
                    ...
                    <div class="col-6 pt-2">
                        ...
                        <div class="tags">
                            <div class="row g-2">
                                ...
                                @endif
                                <!--<div class="col-auto">-->
                                <!--    <a href="javascript:void(0)" class="card-tag"><i class="icon-laptop-mac"></i><span>Remote-friendly</span></a>-->
                                <!--</div>-->
                                <!--<div class="col-auto">-->
                                <!--    <a href="javascript:void(0)" class="card-tag"><i class="icon-home-pin"></i><span>Bengaluru</span></a>-->
                                <!--</div>-->
                            </div>
                        </div>
                        <div class="fs-12">
                            {{ isset($requestParameters['ci_date'], $requestParameters['co_date']) ? \Carbon\Carbon::parse($requestParameters['ci_date'])->format('d') . '-' . \Carbon\Carbon::parse($requestParameters['co_date'])->format('d M') : '' }}
                            &bull; {{ isset($requestParameters['tot_no_of_days']) ? $requestParameters['tot_no_of_days'] . ' ' . ($requestParameters['tot_no_of_days'] == 1 ? 'night' : 'nights') : '' }}
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="table-subtotal">
                            <table class="table mb-0 table-borderless">
                                <tr>
                                    <td>&#8377;{{ isset($requestParameters['price_per_night']) ? number_format($requestParameters['price_per_night']) : '' }} x {{ isset($requestParameters['tot_no_of_days']) ? $requestParameters['tot_no_of_days'] . ' ' . ($requestParameters['tot_no_of_days'] == 1 ? 'night' : 'nights') : '' }} </td>
                                    <td align="right">&#8377;{{ number_format($requestParameters['price_per_night']*$requestParameters['tot_no_of_days']) }}</td>
                                    <td></td>
                                </tr>
                                @if($requestParameters['total_extra_guest_charge'] > 0)
                                <tr>
                                    <td>Extra charge ( {{ $requestParameters['extra_guest_charge'] }} x {{ $requestParameters['tot_no_of_days'] }} )</td>
                                    <td align="right">&#8377;{{ $requestParameters['total_extra_guest_charge'] }}</td>
                                    <td></td>
                                </tr>
                                @endif
                                @if(isset($requestParameters['additionalCharges']) && $requestParameters['additionalCharges'])
                                @foreach($requestParameters['additionalCharges'] as $additionalCharge)
                                <tr>
                                    <td>{{ $additionalCharge['name'] }} </td>
                                    <td align="right">
                                        @if($additionalCharge['type_option'] == 'Per_Stay')
                                        &#8377;{{ number_format($additionalCharge['price']) }}
                                        @else
                                        &#8377;{{ number_format($additionalCharge['price']*$requestParameters['tot_no_of_days']) }}
                                        @endif
                                    </td>
                                    <td></td>
                                </tr>
                                @endforeach
                                @endif
                                @if($requestParameters['total_pet_charge'] > 0)
                                <tr>
                                    <td>Pet Fees (&#8377;{{ $requestParameters['per_pet_charge'] }} x {{ $requestParameters['pets'] }})</td>
                                    <td align="right">&#8377;{{ $requestParameters['total_pet_charge'] }}</td>
                                    <td></td>
                                </tr>
                                @endif
                                @if($requestParameters['discountCode'] && $requestParameters['discountAmount'])
                                <tr class="promo-code-tr">
                                    <td class="text-link">Promo Code {{ $requestParameters['discountCode']  }}</td>
                                    <td align="right" class="text-link">-&#8377;{{ $requestParameters['discountAmount']  }}</td>
                                    <td align="right"></td>
                                </tr>
                                @endif()
                                <tr>
                                    <td>Govt. Taxes ({{ $requestParameters['tax'] }}%)</td>
                                    <td align="right">&#8377;{{ $requestParameters['formatted_total_taxable_amount'] }}</td>
                                    <td></td>
                                </tr>
                                <!--<tr class="promo-code-tr">-->
                                <!--    <td class="text-link">Promo Code AX43434</td>-->
                                <!--    <td align="right" class="text-link">-&#8377;3,472</td>-->
                                <!--    <td align="right">-->
                                <!--        <a href="javascript:void(0);" class="cus-link"><i class="icon-minus"></i> Remove</a>-->
                                <!--    </td>-->
                                <!--</tr>-->
                            </table>
                        </div>
                        <table class="table mb-0 table-borderless">
                            <tr class="fw-bold">
                                ...
                                <td align="right">&#8377;{{ $requestParameters['num_formatted_tot_price'] }}</td>
                            </tr>
                        </table>
                ...
                <div class="row">
                    ...
                    <div class="col-12">
                        <form action="" id="yourFormId">
                            ...
                            <input type="hidden" name="checkInDate" value="{{ request()->input('checkin') }}">
                            <input type="hidden" name="checkOutDate" value="{{request()->input('checkout')}}">
                            <input type="hidden" name="per_night_price" value="{{$property->per_night_price}}">
                            <input type="hidden" name="channel" id="channel" value="Website">
                            <input type="hidden" name="noOfNights" value="{{ request()->input('tot_no_of_days') }}">
                            <input type="hidden" name="taxAmount" value="{{ number_format(round(($property->per_night_price*18 )/100), 0) }}">
                            <input type="hidden" name="netPayableAmount" value="{{ round(($property->per_night_price * $property->noOfNights) * 1.18) }}">
                            <input type="hidden" name="ptype" id="ptype" value="{{ $property->ptype }}">
                            {{-- <input type="hidden" name="state" id="state" class="form-control " value="{{ $userData->state ?? ''}}" readonly>
                            <input type="hidden" name="country_code" id="country_code" class="form-control " value="{{ $userData->country_code ?? ''}}" readonly> --}}
                            <div class="row g-4">
                                ...
                                <div class="col-12">
                                    <div class="form-group cs-input">
                                        ...
                                        <input type="text" name="address_line_2" id="address_line_2" class="form-control" placeholder="Sector, Town/city" value="{{ $userData->address_2 ?? ''}}">
                                    </div>
                                </div>
                                ...
                                <div class="col-12 col-md-4 text-md-end">
                                    <div class="form-group cs-input text-md-end">
                                        <a href="{{ route('customer.login') }}{{ request()->getQueryString() ? '?' . request()->getQueryString() : '' }}" style="text-decoration:none">Already have an account? Sign In</a>
                                    </div>
                                </div>
                                @endif
                                ...
                                <tbody>
                                    <tr>
                                        <td>
                                            ...
                                            <div> {{ isset($requestParameters['ci_date'], $requestParameters['co_date']) ? \Carbon\Carbon::parse($requestParameters['ci_date'])->format('d') . '-' . \Carbon\Carbon::parse($requestParameters['co_date'])->format('d M') : '' }}</div>
                                        </td>
                                        <th class="text-end">
                                            <a href="{{ route('property-detail',[
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <!--<div>@if($requestParameters['adults'] =='1') {{ $requestParameters['adults'] }} Adult @else {{ $requestParameters['adults'] }} Adults <br>{{ $requestParameters['children'] }} Children <br>{{ $requestParameters['pets'] }} Pet @endif</div>-->
                                            @if($requestParameters['adults'] =='1')
                                            {{ $requestParameters['adults'] }} Adult
                                            @else {{ $requestParameters['adults'] }}
                                            Adults
                                            @endif

                                            @if($requestParameters['children'])
                                            <br>
                                            {{ $requestParameters['children'] }} Children
                                            @endif
                                            @if($requestParameters['pets'])
                                            <br>
                                            {{ $requestParameters['pets'] }} Pet
                                            @endif
                                        </td>
                                        <th class="text-end">
                                            <a href="{{ route('property-detail',[
                                    ...
                                    <tr>
                                        <td colspan="2">{{ $requestParameters['adults'] + $requestParameters['children'] }} guests total</th>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                ...
                if (response['status'] == true) {
                    ...
                    if (response.orderId) {
                        ...
                        var options = {
                            ...
                            "modal": {
                                "ondismiss": function() {
                                    window.location.replace("{{ route('payment-failure') }}?orderId=" + orderId);
                                }
                            }
                        };
